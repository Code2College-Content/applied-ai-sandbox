...
def test_empty_title_shows_error(client, auth):
    auth.login()
...
def test_empty_body_shows_error(client, auth):
    auth.login()
...
def test_valid_submit_redirects_and_persists(client, app, auth):
    auth.login()
    app.notes.clear()
    r = client.post(
    ...
    assert any(
        n["title"] == "First" and n["owner"] == "1" for n in app.notes
    )


def test_invalid_submit_preserves_typed_values(client, auth):
    auth.login()
