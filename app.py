...
def create_app() -> Flask:
    ...
    def new_note():
        if request.method == "POST":
            ...
            if not title:
                return render_template("new_note.html", title=title, body=body, error="Title is required")
            if not body:
                return render_template("new_note.html", title=title, body=body, error="Body is required")
